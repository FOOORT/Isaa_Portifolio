import React from "react";

const MaskedAbout = () => {
  return (
    <section className="flex flex-col gap-2 mt-14 lg:mt-20 w-full lg:w-[80vw] text-3xl bg-green p-12 rounded-lg">
      <p className="font-extralight leading-[42px]">
        I AM
        <span className="text-white"> PROFESSIONAL DEVELOPER </span> DEVELOPER
        WITH STRONG FOCUS ON PRODUCING HIGH QUALITY & IMPACTIFULLY DIGITAL
        EXPERIENCE ABOUT ME I AM PRO SKILLED PRODUCT DESIGNER WITH FOCUS ON
        PRODUCING HIGH QUALITY & IMPACTIFULLY DIGITAL EXPERIENCE
      </p>
    </section>
  );
};

export default MaskedAbout;
